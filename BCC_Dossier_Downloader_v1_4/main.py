...
class App:
    def __init__(self, root: tk.Tk):
        ...
        ttk.Label(frame, text="BIN (direct entry):").grid(row=0, column=0, sticky="w")
        ttk.Entry(frame, textvariable=self.direct_bin).grid(row=1, column=0, sticky="ew", padx=(0, 8))
        ttk.Label(frame, text="Номер досье (optional but recommended):").grid(row=0, column=1, sticky="w")
        ttk.Entry(frame, textvariable=self.dossier_number).grid(row=1, column=1, sticky="ew")

        ttk.Label(frame, text="Or BIN source for batch mode (.xlsx, .csv or .txt):").grid(row=2, column=0, sticky="w", pady=(12, 0))
        ttk.Entry(frame, textvariable=self.bin_file).grid(row=3, column=0, sticky="ew", padx=(0, 8))
        ttk.Button(frame, text="Browse…", command=self.pick_bin).grid(row=3, column=1, sticky="ew")

        ttk.Label(frame, text="Download folder:").grid(row=4, column=0, sticky="w", pady=(12, 0))
        ttk.Entry(frame, textvariable=self.output_dir).grid(row=5, column=0, sticky="ew", padx=(0, 8))
        ttk.Button(frame, text="Browse…", command=self.pick_output).grid(row=5, column=1, sticky="ew")

        ttk.Label(frame, text="BCC username (not saved):").grid(row=6, column=0, sticky="w", pady=(12, 0))
        ttk.Entry(frame, textvariable=self.username).grid(row=7, column=0, sticky="ew", padx=(0, 8))
        ttk.Label(frame, text="BCC password (not saved):").grid(row=6, column=1, sticky="w", pady=(12, 0))
        ttk.Entry(frame, textvariable=self.password, show="•").grid(row=7, column=1, sticky="ew")

        ttk.Label(frame, text="BCC dossier search URL:").grid(row=8, column=0, sticky="w", pady=(12, 0))
        ttk.Entry(frame, textvariable=self.target_url).grid(row=9, column=0, columnspan=2, sticky="ew")

        ttk.Label(frame, text="Browser:").grid(row=10, column=0, sticky="w", pady=(12, 0))
        ttk.Combobox(
        ...
        info = (
            ...
            "Only two document types are downloaded: Заявление о присоединении (договор лизинга) and Договор купли-продажи."
        )
        ttk.Label(frame, text=info, wraplength=760).grid(row=12, column=0, columnspan=2, sticky="w", pady=(14, 8))
        ...
        ttk.Label(frame, text="Progress / log:").grid(row=14, column=0, sticky="w", pady=(14, 4))
    ...
    def start(self):
        ...
        if direct:
            ...
            if not source_text:
                messagebox.showerror("BIN", "Enter a 12-digit BIN directly, or select a BIN list file for batch mode.")
                return
        ...
        dossier_number = self.dossier_number.get().strip()
        threading.Thread(target=self.worker, args=(bins, out, username, password, dossier_number), daemon=True).start()
    ...
    def worker(self, bins: list[str], out: Path, username: str, password: str, dossier_number: str):
        try:
            with sync_playwright() as p:
                ...
                downloader.download_bins(bins, dossier_number=dossier_number)
