...
def document_kind(document: Document) -> str | None:
    """Return the exact business contract type wanted by the downloader.

    BCC can group acts under a broad ``Наименование документа`` such as
    ``Договор купли-продажи``.  The business rule is stricter: the
    ``Примечание`` (``Document.title``) itself must identify the contract.
    Acts and other derivative documents are explicitly rejected even when
    their category contains the word ``Договор``.
    """
    note = normalize_match_text(document.title)

    # Never treat derivative/acceptance documents as the contract itself.
    # This intentionally focuses on the BCC "Примечание" field.
    reject_terms = (
        "акт",
    ...
    # Leasing contract: support both the captured accession-statement wording
    # and rows where the note directly says financial leasing agreement.
    if (
        ("заявление о присоединении" in note and "договор лизинга" in note)
        or "договор финансового лизинга" in note
        or "договор лизинга" in note
    ):
        return "leasing_contract"

    # Purchase/sale contract itself (including vehicle wording), but not acts.
    if "договор купли продажи" in note:
