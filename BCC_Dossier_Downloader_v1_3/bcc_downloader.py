...
def dossier_matches(dossier: Dossier, requested_number: str) -> bool:
    ...
    if not wanted:
        return True
    return wanted in {normalize_match_text(dossier.dossier_id), normalize_match_text(dossier.label)} or wanted in normalize_match_text(dossier.label)
...
class BCCDownloader:
    ...
    def _post(self, endpoint: str, data: dict[str, str]) -> str:
        result = self._browser_fetch(self.base + endpoint, method="POST", data=data, binary=False)
