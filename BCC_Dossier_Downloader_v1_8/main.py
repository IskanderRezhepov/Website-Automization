...
class App:
    def __init__(self, root: tk.Tk):
        self.root = root
        root.title("BCC Dossier Downloader")
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            outer,
            text="Download only the leasing agreement/application and purchase-sale agreement for one case.",
        ...
        docs = ttk.LabelFrame(outer, text="Required documents", padding=12)
        docs.grid(row=6, column=0, sticky="ew", pady=(12, 0))
        docs.columnconfigure(0, weight=1)
        docs.columnconfigure(1, weight=1)
        ttk.Label(docs, text="Leasing contract / Заявление о присоединении").grid(row=0, column=0, sticky="w")
        ttk.Label(docs, textvariable=self.leasing_status, font=("Segoe UI", 11, "bold")).grid(row=1, column=0, sticky="w")
        ttk.Label(docs, text="Purchase agreement / Договор купли-продажи").grid(row=0, column=1, sticky="w")
        ...
        toggles.grid(row=7, column=0, sticky="ew", pady=(10, 0))
    ...
    def toggle_advanced(self):
        if self.show_advanced.get():
            self.advanced.grid(row=8, column=0, sticky="ew", pady=(8, 0))
        else:
            self.advanced.grid_remove()

    def toggle_details(self):
        if self.show_details.get():
            self.details.grid(row=9, column=0, sticky="nsew", pady=(8, 0))
        else:
    ...
    def worker(self, bins: list[str], out: Path, username: str, password: str, dossier_number: str, customer_key: str):
        ...
        try:
            with sync_playwright() as p:
                ...
                self.messages.put(("log", f"Finished. Report: {out / 'download_log.xlsx'}"))
