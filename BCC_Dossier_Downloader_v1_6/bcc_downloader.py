...
def document_kind(document: Document) -> str | None:
    """Return the business document type used by the downloader/status report."""
    text = normalize_match_text(" ".join([document.category, document.title]))
    if "заявление о присоединении" in text and "договор лизинга" in text:
        return "leasing_contract"
    if "договор купли продажи" in text:
