...
from .table_review import apply_table_review
...
def _append_manual_fields(updated: dict, form_data, now: str) -> int:
    ...
    for doc_index, document in enumerate(updated.get("documents", [])):
        field_names = values_for(f"add_{doc_index}_field_name")
        values = values_for(f"add_{doc_index}_value")
        pages = values_for(f"add_{doc_index}_page")
        custom_labels = values_for(f"add_{doc_index}_custom_label")
        notes = values_for(f"add_{doc_index}_notes")
        candidate_sources = values_for(f"add_{doc_index}_candidate_source")
...
def apply_review(result: dict, form_data) -> tuple[dict, int]:
    ...
    updated, table_changes = apply_table_review(updated, form_data, now)
    changed_count += table_changes

    updated["review"] = {
        "updated_at": now,
        "changed_fields": changed_count,
        "added_fields": added_count,
        "changed_table_rows": table_changes,
