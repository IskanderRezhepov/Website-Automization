...
def _safe_output_stem(value: str, limit: int = 20) -> str:
...
def individual_output_path(source: Path, target_iin_bin: str = "") -> Path:
    """Return a short deterministic Excel path belonging to ``source``.

    The client BIN is intentionally not repeated in the filename: the workbook
    already lives inside that client's tree and the source-stem hash guarantees
    uniqueness.  Keeping this short avoids the classic Windows MAX_PATH limit.
    """
    filename = f"АНАЛИЗ_{_safe_output_stem(source.stem)}.xlsx"
            ...
            for item in document.get("fields", []):
                ...
                for quoted in reversed(quoted_names):
                    ...
                    if not re.match(r"^(?:ТОО|ЖШС|ИП)\b", cleaned, re.I):
                        cleaned_core = cleaned.strip('«»" ')
                        cleaned = f"{legal} «{cleaned_core}»"
                    name_candidates.append(cleaned)
                    break
...
def run_folder_batch(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="credit_dossier_batch_") as temp_dir:
        ...
        # A failed source must still receive a machine production decision.
        # This is especially important when OCR is unavailable: unattended mode
        # quarantines the document instead of silently omitting it from the gate.
        from .production_mode import route_failed_source
        for failed_item in items:
            if failed_item.error:
                production_records.append(
                    route_failed_source(folder, failed_item.source, failed_item.error)
                )
        ...
        if production_records:
            from .production_mode import enforce_pair_atomicity, write_manifest
            production_records = enforce_pair_atomicity(folder, production_records)
