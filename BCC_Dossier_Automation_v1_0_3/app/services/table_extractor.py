...
EQUIPMENT_TYPES: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("Легковой автомобиль", ("ЛЕГКОВОЙ АВТОМОБИЛЬ", "АВТОМОБИЛЬ ЛЕГКОВОЙ", "HYUNDAI SANTA FE", "JAC SUNRAY")),
...
MODEL_PATTERNS = (
    ...
    re.compile(r"\b(?:JAC|HOWO|SHACMAN|SITRAK|XCMG|SANY|ZOOMLION|LIUGONG|SDLG|VOLVO|KOMATSU|CATERPILLAR|CAT|HYUNDAI|DOOSAN|MAN|DAF|SCANIA|KAMAZ|КАМАЗ)\s+[A-Z0-9][A-Z0-9 ._／/\-]{1,45}\b", re.I),
)
...
def _subsidy_schedule_rows(document: ReadDocument) -> dict | None:
    """Extract the five-amount subsidy schedule printed in column order."""
    rows = []
    date_token = re.compile(r"^\d{2}[.\-/]\d{2}[.\-/]\d{2,4}$")
    ...
    for page in document.pages:
        ...
        for index, line in enumerate(lines):
            if not date_token.fullmatch(line):
                continue
            parsed_date = _date(line)
            if not parsed_date:
                continue
            values = []
            for follow in lines[index + 1:index + 8]:
                if date_token.fullmatch(follow):
                    break
                if money_line.fullmatch(follow):
                    parsed = parse_money(follow)
            ...
            rows.append({
                "date": parsed_date,
                "balance_kzt": values[0],
                "principal_kzt": values[1],
                "subsidized_interest_kzt": values[2],
                "recipient_interest_kzt": values[3],
                "total_interest_kzt": values[4],
                "page": page.page_number,
                "source_method": page.extraction_method,
            })
    by_date = {}
    ...
    rows = sorted(
        by_date.values(),
        key=lambda row: datetime.strptime(row["date"], "%d.%m.%Y"),
    )
    if len(rows) < 5:
        return None

    principal = sum(Decimal(str(row["principal_kzt"])) for row in rows)
    subsidized = sum(
        Decimal(str(row["subsidized_interest_kzt"])) for row in rows
    )
    recipient = sum(
        Decimal(str(row["recipient_interest_kzt"])) for row in rows
    )
    total_interest = sum(
        Decimal(str(row["total_interest_kzt"])) for row in rows
    )
    reconciled = abs((subsidized + recipient) - total_interest) <= Decimal("1")
    total_row = {
        "date": "Итого",
        "balance_kzt": None,
        "principal_kzt": float(principal),
        "subsidized_interest_kzt": float(subsidized),
        "recipient_interest_kzt": float(recipient),
        "total_interest_kzt": float(total_interest),
        "page": None,
        "source_method": "calculated",
    }
    return {
        "name": "subsidy_payment_schedule_rows",
        "label_ru": "График субсидирования",
        "columns": [
            {"key": "date", "label_ru": "Дата платежа"},
            {"key": "balance_kzt", "label_ru": "Остаток основного долга, тенге"},
            {"key": "principal_kzt", "label_ru": "Погашение основного долга, тенге"},
            {
                "key": "subsidized_interest_kzt",
                "label_ru": "Субсидируемое вознаграждение, тенге",
            },
            {
                "key": "recipient_interest_kzt",
                "label_ru": "Вознаграждение получателя, тенге",
            },
            {
                "key": "total_interest_kzt",
                "label_ru": "Итоговое вознаграждение, тенге",
            },
            {"key": "page", "label_ru": "Страница"},
        ],
        "rows": rows + [total_row],
        "row_count": len(rows),
        "summary": {
            "principal_sum_kzt": float(principal),
            "subsidized_interest_sum_kzt": float(subsidized),
            "recipient_interest_sum_kzt": float(recipient),
            "total_interest_sum_kzt": float(total_interest),
            "first_payment_date": rows[0]["date"],
            "last_payment_date": rows[-1]["date"],
            "interest_totals_reconciled": reconciled,
        },
        "confidence": .98 if len(rows) >= 30 and reconciled else .86,
        "status": "extracted" if len(rows) >= 30 and reconciled else "candidate",
        "notes": (
            "Строки восстановлены из пяти денежных колонок графика; "
            "итоговое вознаграждение сверено как сумма двух частей."
        ),
    }
...
def _asset_rows(document: ReadDocument) -> dict | None:
    ...
    for page in document.pages:
        ...
        for index, line in enumerate(lines):
            if re.match(r"^(?:ИТОГО|ВСЕГО|БАРЛЫҒЫ|ЖАЛПЫ)\b", line.upper()):
                continue
...
def extract_tables(document: ReadDocument, document_type: str) -> list[dict]:
    ...
    Subsidy schedules are accepted only through the layout-aware parser (or the
    same guarded row parser used for standalone schedules). This preserves the
    37-row appendix and its totals without treating arbitrary prose as a table.
    """
    tables: list[dict] = []
    if document_type in {'payment_schedule', 'addendum', 'subsidy_agreement'}:
        schedule = (
            _subsidy_schedule_rows(document)
            if document_type == "subsidy_agreement"
            else schedule_from_layout(document) or _schedule_rows(document)
        )
        if schedule:
