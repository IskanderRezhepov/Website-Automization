...
BRANDS = (
    "HYUNDAI", "GEELY", "ГАЗ", "JAC", "XCMG", "SHACMAN", "HOWO", "SITRAK", "SANY",
...
def _explicit_value(context: str, labels: tuple[str, ...]) -> str | None:
    header_artifacts = {
        ...
        "КОЛИЧЕСТВО", "КОЛИЧЕСТВО ЕДИНИЦ", "САНЫ",
    }
    for label in labels:
        match = re.search(rf"{label}\s*[:\-]?\s*([^\n|;,]{{2,60}})", context, re.I)
        if match:
            value = re.sub(r"\s+", " ", match.group(1)).strip(" ,.;:-")
            normalized = re.sub(r"[^0-9A-ZА-ЯЁӘІҢҒҮҰҚӨҺ ]", "", value.upper())
            if normalized in header_artifacts:
                continue
            return value
    return None


def _contains_normalized(haystack: object, needle: object) -> bool:
    def compact(value: object) -> str:
        return re.sub(r"[^0-9A-ZА-ЯЁӘІҢҒҮҰҚӨҺ]", "", str(value or "").upper())

    expected = compact(needle)
    return bool(expected) and expected in compact(haystack)
...
def improve_equipment_tables(document, fields: list[dict], tables: list[dict]) -> list[dict]:
    ...
    contract_value = next((
        ...
        for item in fields
        if item.get("name") in {
            "lease_asset_value_kzt", "purchase_total_kzt", "equipment_total_kzt",
            "total_amount_kzt", "act_total_amount_kzt",
    ...
    for table in result:
        ...
        for row in rows:
            # Complete flattened/specification rows from the final normalized
            # document fields. These values have already passed the field-level
            ...
            field_mapping = {
                ...
                "unit_price_kzt": "equipment_unit_price_kzt",
                "total_amount_kzt": "equipment_total_kzt",
            }
            for row_key, field_key in field_mapping.items():
                value = final_values.get(field_key)
                if value not in (None, "") and row.get(row_key) in (None, ""):
                    row[row_key] = value
            # Older parsers used the table-oriented field name. Keep it as a
            # compatibility fallback, but prefer the public catalog name above.
            if row.get("unit_price_kzt") in (None, ""):
                row["unit_price_kzt"] = final_values.get("unit_price_kzt")

            # A serial/chassis identifier is authoritative for equipment that
            # has no 17-character road-vehicle VIN. Expose it through the
            # common identifier column before dossier-level deduplication.
            if row.get("vin") in (None, ""):
                row["vin"] = (
                    row.get("serial_number")
                    or final_values.get("serial_number")
                    or final_values.get("equipment_identifier")
                )
            ...
            targeted_model = (
                row.get("source_method") == "targeted"
                and row.get("model") not in (None, "")
            )

            if brand:
                row["brand"] = brand
                row["manufacturer"] = row.get("manufacturer") or brand
            if model and not targeted_model:
                row["model"] = model
            ...
            if row.get("equipment_type") and str(row["equipment_type"]).upper().startswith("VIN"):
                row["equipment_type"] = equipment_type
            explicit_year = _explicit_year(context)
            explicit_color = _explicit_value(context, ("ЦВЕТ", "ТҮСІ"))
            explicit_country = _explicit_value(
                context, ("СТРАНА ПРОИСХОЖДЕНИЯ", "СТРАНА ИЗГОТОВЛЕНИЯ", "ШЫҒАРЫЛҒАН ЕЛ")
            )
            targeted_year = (
                row.get("source_method") == "targeted"
                and _contains_normalized(row.get("raw"), row.get("manufacture_year"))
            )
            targeted_color = (
                row.get("source_method") == "targeted"
                and (
                    _contains_normalized(row.get("raw"), row.get("color"))
                    or (
                        str(row.get("color") or "").casefold() == "темно-синий"
                        and bool(re.search(
                            r"\bтемно\s*-",
                            str(row.get("raw") or ""),
                            re.I,
            ...
            if explicit_year:
                row["manufacture_year"] = explicit_year
            elif not targeted_year:
                row["manufacture_year"] = None
            if explicit_color and not targeted_color:
                row["color"] = explicit_color
            elif not explicit_color and not targeted_color:
                row["color"] = None
            if explicit_country:
                row["country_of_origin"] = explicit_country
        ...
        if len(rows) == 1 and contract_value:
            row = rows[0]
            ...
            elif row.get("unit_price_kzt") is None and int(row.get("quantity") or 1) == 1:
