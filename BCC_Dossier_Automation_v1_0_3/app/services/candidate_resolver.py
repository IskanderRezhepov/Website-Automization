...
ROLE_RULES = {
    ...
    "pledgor": {
        "label": "Залогодатель",
        "field": "pledgor_iin_bin",
        "keywords": ("ЗАЛОГОДАТЕЛЬ", "КЕПІЛ БЕРУШІ"),
    },
    "sender": {
        "label": "Отправитель",
        "field": "sender_iin_bin",
        "keywords": ("ОТПРАВИТЕЛЬ", "ЖӨНЕЛТУШІ"),
    },
}
...
def _promote_ibans(document: ReadDocument) -> tuple[list[dict], set[str]]:
    occurrences = _occurrences(document, r"\bKZ\d{2}[0-9A-Z]{16}\b")
    ranked_all: list[tuple[float, str, dict]] = []

    for item in occurrences:
        quote = item["quote"].upper()
        for role, config in ROLE_RULES.items():
            score = _score_role(item["quote"], role)
            if any(token in quote for token in ("IBAN", "ИИК", "ЖСК", "СЧЕТ", "СЧЁТ")):
                score += 0.12

            # Explicit account ownership phrases outweigh generic nearby names.
            explicit_patterns = {
                "lessee": ("СЧЕТ ЛИЗИНГОПОЛУЧАТЕЛ", "ЛИЗИНГ АЛУШЫНЫҢ ШОТ", "СЧЕТ ЗАЕМЩИКА"),
                "lessor": ("СЧЕТ ЛИЗИНГОДАТЕЛ", "ЛИЗИНГ БЕРУШІНІҢ ШОТ"),
                "guarantor": ("ГАРАНТ", "КЕПІЛГЕР"),
                "recipient": ("ПОЛУЧАТЕЛ", "АЛУШЫ"),
                "seller": ("ПРОДАВЕЦ", "САТУШЫ"),
                "sender": ("ОТПРАВИТЕЛ", "ЖӨНЕЛТУШІ"),
            }
            if any(pattern in quote for pattern in explicit_patterns.get(role, ())):
                score += 0.38

            ranked_all.append((min(score, 1.0), role, item))
    ...
    for score, role, item in ranked_all:
        value = item["value"]
        if score < 0.72 or value in used_values or role in used_roles:
            continue

        alternatives = [
            other_score for other_score, other_role, other_item in ranked_all
            if other_item["value"] == value and other_role != role
        ]
        if alternatives and score - max(alternatives) < 0.10:
            continue
        ...
        promoted.append(field(
            ...
            confidence=max(0.72, min(0.96, score)),
            extraction_method=item["method"],
            status="extracted" if score >= 0.84 else "candidate",
            notes=f"Определено по явному заголовку или ближайшему блоку реквизитов стороны «{config['label']}».",
        ))
        used_values.add(value)
        used_roles.add(role)

    return promoted, used_values



def _role_suggestions(document: ReadDocument, value: str, *, iban: bool = False) -> list[dict]:
    pattern = r"\b" + re.escape(value) + r"\b"
    occurrences = _occurrences(document, pattern)
    suggestions: list[dict] = []
    for role, config in ROLE_RULES.items():
        best_score = 0.0
        best_item = None
        for item in occurrences:
            score = _score_role(item["quote"], role) if iban else _explicit_role_score(item["quote"], role, value)
            if iban and any(token in item["quote"].upper() for token in ("IBAN", "ИИК", "СЧЕТ", "СЧЁТ")):
                score += 0.10
            if score > best_score:
                best_score = min(score, 1.0)
                best_item = item
        if best_item is not None and best_score >= 0.28:
            suggestions.append({
                "field_name": f"{role}_iban" if iban else config["field"],
                "label_ru": f"IBAN — {config['label']}" if iban else f"ИИН/БИН — {config['label']}",
                "score": round(best_score, 2),
                "page": best_item["page"],
                "quote": best_item["quote"],
            })
...
def resolve_candidates(document: ReadDocument, fields: list[dict], document_type: str | None = None) -> list[dict]:
    ...
    existing_specific_ids = {
        str(item.get('value')) for item in kept
        if ('iin_bin' in str(item.get('name') or '') or str(item.get('name') or '').endswith('_bin'))
        and not isinstance(item.get('value'), list)
    }
    unresolved_ids = [value for value in all_ids if value not in used_ids and value not in existing_specific_ids]

    # Electronic-signature certificates contain the signer's IIN in a service
    # block.  It is not an unidentified party to the contract and must not
    # block the workbook readiness status.
    filtered_unresolved_ids = []
    for value in unresolved_ids:
        occurrences = _occurrences(document, rf"\b{re.escape(value)}\b")
        quotes = " ".join(str(item.get("quote") or "") for item in occurrences).upper()
        signature_only = bool(occurrences) and all(
            any(token in str(item.get("quote") or "").upper() for token in (
                "ПОДПИСАЛ(А)", "ДАТА ФОРМИРОВАНИЯ ПОДПИСИ",
                "SIGEX.KZ", "ЭЛЕКТРОННАЯ ЦИФРОВАЯ ПОДПИСЬ",
            ))
            for item in occurrences
        )
        if signature_only and ("BCC LEASING" in quotes or "БАНК ЦЕНТРКРЕДИТ" in quotes):
            continue
        filtered_unresolved_ids.append(value)
    unresolved_ids = filtered_unresolved_ids
    if unresolved_ids:
        ...
        candidate_field["suggestions"] = {
            value: _role_suggestions(document, value)
            for value in unresolved_ids[:20]
        }
        kept.append(candidate_field)

    all_ibans = sorted(set(item["value"] for item in _occurrences(document, r"\bKZ\d{2}[0-9A-Z]{16}\b")))
    existing_specific_ibans = {
        str(item.get('value')) for item in kept
        if str(item.get('name') or '').endswith('_iban') and not isinstance(item.get('value'), list)
    }
    unresolved_ibans = [value for value in all_ibans if value not in used_ibans and value not in existing_specific_ibans]
    if unresolved_ibans:
        candidate_field = field(
        ...
        candidate_field["suggestions"] = {
            value: _role_suggestions(document, value, iban=True)
            for value in unresolved_ibans[:20]
        }
        kept.append(candidate_field)
