...
def find_first(document: ReadDocument, *, patterns: Iterable[str], name: str, label_ru: str,
    ...
    for page in page_iter:
        for pattern in patterns:
            ...
            if not match and isinstance(pattern, str) and "\\" in pattern:
                # Backward compatibility for older tests/config files that
                # accidentally stored regex tokens as ``\\s``/``\\d``.
                repaired = pattern.replace(r"\\s", r"\s").replace(r"\\d", r"\d").replace(r"\\b", r"\b")
                if repaired != pattern:
                    match = re.search(repaired, page.text, re.IGNORECASE | re.DOTALL)
            if not match:
                continue
            ...
            # A regex can match only whitespace around a label. Converters such
            # as parse_money then return None. Such a match is not a real field.
            if value is None:
                continue
...
def generic_identifiers(document: ReadDocument) -> list[dict]:
    ...
    for page in document.pages:
        iban_spans = [
            match.span()
            for match in re.finditer(r'\bKZ[0-9A-Z]{18}\b', page.text, re.I)
        ]
        for match in re.finditer(r'(?<!\d)(\d{12})(?!\d)', page.text):
            if any(start <= match.start() and match.end() <= end for start, end in iban_spans):
                continue
    ...
    vins = [
        value
        for value in find_all_regex(document, r'\b[A-HJ-NPR-Z0-9]{17}\b')
        if re.search(r'\d', value)
        and value == value.upper()
    ]
    if vins:
        result.append(field(name='vin_candidates', label_ru='Кандидаты VIN', value=sorted(set(vins)), page=None, quote=None,
    ...
    if doc_ids:
        doc_id_quote = next(
            (
                match.group(0)
                for page in document.pages
                for match in [re.search(r'DOC\s*ID\s*[A-Z0-9]{15,40}', page.text, re.I)]
                if match
            ),
            None,
        )
        result.append(field(name='doc_ids', label_ru='DOC ID', value=doc_ids, page=None, quote=doc_id_quote,
