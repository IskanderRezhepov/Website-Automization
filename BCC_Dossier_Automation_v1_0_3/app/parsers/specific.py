...
def _party(document: ReadDocument, role: str, name: str, label_ru: str) -> dict | None:
    ...
    patterns_by_role = {
        'ПРОДАВЕЦ': [
            ...
            r'САТУШЫ\s*/\s*ПРОДАВЕЦ\s*:\s*[\s\S]{0,180}?(Товарищество\s+с\s+ограниченной\s+ответственностью\s*[«"][^»"\n]{2,100}[»"])',
            r'((?:ТОО|Товарищество\s+с\s+ограниченной\s+ответственностью)\s*[«"]ANTO\s+MOTORS[»"])',
        ],
        'ПОКУПАТЕЛЬ': [
            ...
            r'САТЫП\s+АЛУШЫ\s*/\s*ПОКУПАТЕЛЬ\s*:\s*[\s\S]{0,220}?(Дочерняя\s+компания\s+АО\s*[«"]Банк\s+ЦентрКредит[»"]\s+АО\s*[«"]BCC\s+Leasing[»"])',
            r'(Дочерняя\s+компания\s+АО\s*[«"]Банк\s+ЦентрКредит[»"]\s+АО\s*[«"]BCC\s+Leasing[»"])',
        ],
        'ЛИЗИНГОПОЛУЧАТЕЛЬ': [
    ...
    result = find_first(
        document, patterns=patterns, name=name, label_ru=label_ru,
        confidence=0.9, pages=4,
    )
    if not result:
        return None

    value = re.sub(r"\s+", " ", str(result.get("value") or "")).strip(" ,;/")
    # Prefer the Russian legal name when a bilingual requisites row contains
    # both Kazakh and Russian forms.  Keep the role, not merely a nearby party.
    ...
    if legal_matches:
        legal_form, legal_name = legal_matches[-1]
        short_form = {
            "товарищество с ограниченной ответственностью": "ТОО",
            "акционерное общество": "АО",
            "индивидуальный предприниматель": "ИП",
        }.get(legal_form.lower(), legal_form.upper())
        value = f"{short_form} «{legal_name.strip()}»"
    else:
        quoted = re.findall(r"[«\"]([^»\"]{2,100})[»\"]", value)
        if quoted:
            preferred = next(
                (candidate for candidate in reversed(quoted) if re.search(r"[A-Za-zА-Яа-я]", candidate)),
                quoted[-1],
            )
            default_form = {
                "ПРОДАВЕЦ": "ТОО",
                "ПОКУПАТЕЛЬ": "АО",
                "ЛИЗИНГОПОЛУЧАТЕЛЬ": "ИП",
            }.get(role_upper, "")
            value = f"{default_form} «{preferred.strip()}»".strip()
    result["value"] = value
...
def parse_addendum(document: ReadDocument) -> list[dict]:
    ...
    if tranches:
        fields.append(field(name='tranche_amounts_kzt', label_ru='Суммы траншей, тенге', value=tranches,
                            ...
                            extraction_method='mixed' if document.used_ocr else 'digital', status='candidate'))
    _append(fields, find_first(document, patterns=[
        r'(?:Дополнительное\s+соглашение|Қосымша\s+келісім)\s*(?:№|N)?\s*([0-9]{1,4})',
    ], name='addendum_number', label_ru='Номер дополнительного соглашения', pages=2, confidence=0.90))
    _append(fields, find_first(document, patterns=[
        r'(?:к\s+Договору\s+субсидирования|Субсидиялау\s+шартына).{0,160}?(?:№|N)\s*([A-ZА-Я0-9./_-]{5,50})',
    ], name='linked_subsidy_contract_number', label_ru='Связанный договор субсидирования', converter=normalize_contract_number, validator=valid_contract_number, pages=3))
    _append(fields, find_first(document, patterns=[
        r'(?:к\s+Договору\s+финансового\s+лизинга|Қаржылық\s+лизинг\s+шартына).{0,160}?(?:№|N)\s*([A-ZА-Я0-9./_-]{5,50})',
    ], name='linked_lease_contract_number', label_ru='Связанный договор финансового лизинга', converter=normalize_contract_number, validator=valid_contract_number, pages=3))
    fields.extend(generic_identifiers(document))
    return _deduplicate(fields)
...
def parse_credit_line_agreement(document: ReadDocument) -> list[dict]:
    ...
    _append(fields, find_first(document, patterns=[r'(?:Цель\s+КЛ|КЖ\s+мақсаты)\s*[:\-]?\s*([^\n]{4,180})'], name='credit_line_purpose', label_ru='Цель кредитной линии', pages=3))
    _append(fields, find_first(document, patterns=[r'(?:Период\s+доступности\s+КЛ|КЖ\s+қолжетімді\s+болатын\s+кезең).{0,120}?(\d{2}[.\-/]\d{2}[.\-/]20\d{2})'], name='availability_end_date', label_ru='Дата окончания периода доступности', pages=4))
    ...
    cadastral = find_all_regex(document, r'\b\d{2}:\d{3}:\d{3}:\d{2,6}\b')
    if cadastral:
        fields.append(field(name='collateral_cadastral_numbers', label_ru='Кадастровые номера залога', value=sorted(set(cadastral))[:50], page=None, quote=None, confidence=0.94 if not document.used_ocr else 0.74, extraction_method='digital' if not document.used_ocr else 'mixed', status='extracted' if not document.used_ocr else 'candidate', notes=f'Найдено {len(set(cadastral))} уникальных кадастровых номеров.'))
    fields.extend(generic_identifiers(document))
    return _deduplicate(fields)
...
def parse_cash_pledge_agreement(document: ReadDocument) -> list[dict]:
    ...
    _append(fields, find_first(document, patterns=[
        r'(?:Заемщик|Залогодатель|Қарыз\s+алушы|Кепіл\s+беруші).{0,220}?(?:БИН|БСН)\s*[:№-]?\s*(\d{12})',
        r'(?:БИН|БСН)\s*[:№-]?\s*(\d{12}).{0,220}?(?:Заемщик|Залогодатель|Қарыз\s+алушы|Кепіл\s+беруші)',
    ], name='pledgor_bin', label_ru='БИН — Залогодатель', pages=3))
    _append(fields, find_first(document, patterns=[
        r'(?:Товарищество\s+с\s+ограниченной\s+ответственностью|ТОО)\s*[«"]([^»"\n]{2,100})[»"]?.{0,220}?(?:Заемщик|Залогодатель)',
    ], name='pledgor_name', label_ru='Залогодатель', pages=3))
    _append(fields, find_first(document, patterns=[
        r'(?:в\s+сумме|сомадағы)\s*(\d[\d \u00a0]*(?:[,.]\d{1,2})?)\s*(?:тенге|теңге).{0,220}?(?:депозит|вклад|кепіл\s+заты)',
        r'(?:денег|ақшаны).{0,180}?(?:в\s+сумме|сомасында)\s*(\d[\d \u00a0]*(?:[,.]\d{1,2})?)\s*(?:тенге|теңге)',
    ], name='pledge_amount_kzt', label_ru='Сумма денежного залога, тенге', converter=parse_money, pages=4))
...
def parse_subsidy_agreement(document: ReadDocument) -> list[dict]:
    ...
    _append(fields, find_first(document, patterns=[
        r'(?:Получатель|Алушы).{0,260}?(?:БИН|БСН)\s*[:№-]?\s*(\d{12})',
        r'(?:БИН|БСН)\s*[:№-]?\s*(\d{12}).{0,260}?(?:Получатель|Алушы)',
    ], name='recipient_bin', label_ru='БИН — Получатель', pages=5))
...
def parse_guarantee_contract(document: ReadDocument) -> list[dict]:
    fields: list[dict] = []
    _append(fields, find_first(document, patterns=[
        r'(?:ДОГОВОР\s+ГАРАНТИИ|КЕПІЛДІК\s+ШАРТЫ).{0,100}?(?:№|N)\s*([A-ZА-Я0-9./_-]{5,50})',
    ], name='guarantee_contract_number', label_ru='Номер договора гарантии',
       converter=normalize_contract_number, validator=valid_contract_number, pages=2))
    _append(fields, find_first(document, patterns=[
        r'(?:Договор[ау]?\s+финансового\s+лизинга|Қаржы\s+лизингі\s+шарты).{0,100}?(?:№|N)\s*([A-ZА-Я0-9./_-]{5,50})',
    ], name='linked_lease_contract_number', label_ru='Связанный договор финансового лизинга',
       converter=normalize_contract_number, validator=valid_contract_number, pages=3))
    _append(fields, find_first(document, patterns=[
        r'(?:основн(?:ого|ой)\s+долг[а]?|негізгі\s+борыш).{0,220}?(\d[\d \u00a0]*(?:[,.]\d{1,2})?)\s*(?:тенге|теңге)',
        r'(?:в\s+размере|мөлшерінде)\s*(\d[\d \u00a0]*(?:[,.]\d{1,2})?)\s*(?:тенге|теңге).{0,120}?(?:месяц|ай)',
    ], name='guarantee_amount_kzt', label_ru='Сумма гарантии, тенге', converter=parse_money, pages=3, confidence=0.86))
    _append(fields, find_first(document, patterns=[
        r'(?:граждан(?:ка|ин)|азаматша)[^\n]{0,140}?(?:ИИН|ЖСН)\s*(\d{12}).{0,100}?(?:Гарант|Кепілгер)',
        r'(?:Гарант|Кепілгер).{0,180}?(?:ИИН|ЖСН)\s*(\d{12})',
    ], name='guarantor_iin', label_ru='ИИН — Гарант', pages=3, confidence=0.90))
    _append(fields, find_first(document, patterns=[
        r'(?:ТОО|Товарищество\s+с\s+ограниченной\s+ответственностью)\s*[«"]([^»"\n]{2,100})[»"]?.{0,220}?(?:Лизингополучатель|Лизинг\s+алушы)',
    ], name='lessee_name', label_ru='Лизингополучатель', pages=3))
    _append(fields, find_first(document, patterns=[
        r'(?:БИН|БСН)\s*(\d{12}).{0,180}?(?:Лизингополучатель|Лизинг\s+алушы)',
    ], name='lessee_bin', label_ru='БИН — Лизингополучатель', pages=3))
    fields.extend(generic_identifiers(document))
    return _deduplicate(fields)


def parse_bank_guarantee_application(document: ReadDocument) -> list[dict]:
    fields: list[dict] = []
    _append(fields, find_first(document, patterns=[
        r'(?:ЗАЯВЛЕНИЕ|ӨТІНІШ).{0,180}?(?:№|N)\s*([A-ZА-Я0-9./_-]{5,50})',
    ], name='guarantee_application_number', label_ru='Номер заявления на гарантию',
       converter=normalize_contract_number, validator=valid_contract_number, pages=2))
    _append(fields, find_first(document, patterns=[
        r'(?:Сумма\s+Гарантии|Кепілдік\s+сомасы)\s*[:\-]?\s*(\d[\d \u00a0]*(?:[,.]\d{1,2})?)',
    ], name='guarantee_amount_kzt', label_ru='Сумма гарантии, тенге', converter=parse_money, pages=2))
    _append(fields, find_first(document, patterns=[
        r'(?:Сумма\s+комиссионного\s+вознаграждения|Комиссиялық\s+сыйақы\s+сомасы)\s*[:\-]?\s*(\d[\d \u00a0]*(?:[,.]\d{1,2})?)',
    ], name='guarantee_commission_kzt', label_ru='Комиссия за гарантию, тенге', converter=parse_money, pages=3))
    _append(fields, find_first(document, patterns=[
        r'(?:Наименование,?\s*БИН\s*Бенефициара|Бенефициар).{0,160}?(\d{12})',
    ], name='beneficiary_bin', label_ru='БИН — Бенефициар', pages=2))
    _append(fields, find_first(document, patterns=[
        r'(?:Товарищество\s+с\s+ограниченной\s+ответственностью|ТОО)\s*[«"]([^»"\n]{2,100})[»"]?.{0,180}?(?:Принципал)',
    ], name='principal_name', label_ru='Принципал', pages=2))
    _append(fields, find_first(document, patterns=[
        r'(?:Принципал).{0,180}?(?:БИН|БСН)\s*(\d{12})',
        r'(?:БИН|БСН)\s*(\d{12}).{0,180}?(?:Принципал)',
    ], name='principal_bin', label_ru='БИН — Принципал', pages=2))
    _append(fields, find_first(document, patterns=[
        r'(?:Срок\s+действия\s+Гарантии).{0,120}?(\d{2}[.\-/]\d{2}[.\-/]\d{4})',
    ], name='guarantee_valid_from', label_ru='Начало срока гарантии', pages=3))
    fields.extend(generic_identifiers(document))
    return _deduplicate(fields)


def parse_direct_debit_agreement(document: ReadDocument) -> list[dict]:
    fields: list[dict] = []

    _append(fields, find_first(
        document,
        patterns=[
            r'(?:СОГЛАШЕНИЕ\s+О\s+ПРЯМОМ\s+ДЕБЕТОВАНИИ\s+БАНКОВСКОГО\s+СЧ[ЕЁ]ТА)'
            r'.{0,180}?(?:№|N)\s*([A-ZА-Я0-9./_-]{4,50})',
        ],
        name='direct_debit_agreement_number',
        label_ru='Номер соглашения о прямом дебетовании',
        converter=normalize_contract_number,
        validator=valid_contract_number,
        pages=2,
    ))
    ...
    _append(fields, find_first(
        document,
        patterns=[
            r'(?:Договор\s+лизинга|Договору\s+лизинга|'
            r'Заявлени\w*\s+о\s+присоединении\s*\(Договор\s+лизинга\))'
            r'.{0,120}?(?:№|N)\s*([A-ZА-Я0-9./_-]{5,50})',
            r'(?:№|N)\s*([A-ZА-Я]{2,5}/\d{4}/[A-ZА-Я]/[A-ZА-Я]/\d{5,8})'
            r'.{0,120}?\(Договор\s+лизинга\)',
        ],
    ...
    _append(fields, find_first(
        document,
        patterns=[
            r'Договор\s+гарантии\s*(?:№|N)\s*([A-ZА-Я0-9./_-]{5,50})',
        ],
        name='linked_guarantee_contract_number',
    ...
    _append(fields, find_first(
        document,
        patterns=[
            r'(?:текущего\s+счета\s+Отправителя|Сч[её]т\s+Отправителя)'
            r'.{0,100}?\b(KZ[0-9A-Z]{18})\b',
            r'\b(KZ[0-9A-Z]{18})\b.{0,150}?(?:Отправитель|Жөнелтуші)',
        ],
        name='sender_iban',
        label_ru='IBAN — Отправитель',
        pages=3,
    ))
    ...
    _append(fields, find_first(
        document,
        patterns=[
            r'(?:Отправитель|Жөнелтуші).{0,260}?(?:БИН|БСН|ИИН|ЖСН)'
            r'\s*[:№-]?\s*(\d{12})',
        ],
        name='sender_iin_bin',
        label_ru='ИИН/БИН — Отправитель',
        pages=4,
    ))
    ...
    _append(fields, find_first(
        document,
        patterns=[
            r'(?:Бенефициар|Бенефициардың\s+атауы).{0,240}?(?:БИН|БСН)'
            r'\s*[:№-]?\s*(\d{12})',
        ],
        name='beneficiary_iin_bin',
        label_ru='БИН — Бенефициар',
        pages=5,
    ))
    ...
    _append(fields, find_first(
        document,
        patterns=[
            r'(?:Наименование\s+Бенефициара|Бенефициардың\s+атауы)\s*[:\-]?\s*'
            r'([^\n]{3,160})',
        ],
...
def parse_real_estate_registration_notice(document: ReadDocument) -> list[dict]:
    fields: list[dict] = []
    _append(fields, find_first(
        document,
        patterns=[r'(\d{2}\.\d{2}\.\d{4})\s+\d{1,2}:\d{2}\s+№'],
        name='registration_notice_date',
        label_ru='Дата уведомления о регистрации',
        pages=2,
    ))
    _append(fields, find_first(
        document,
        patterns=[r'\d{2}\.\d{2}\.\d{4}\s+(\d{1,2}:\d{2})\s+№'],
        name='registration_notice_time',
        label_ru='Время уведомления о регистрации',
        pages=2,
    ))
    _append(fields, find_first(
        document,
        patterns=[r'\d{2}\.\d{2}\.\d{4}\s+\d{1,2}:\d{2}\s+№\s*([0-9-]{6,30})'],
        name='registration_notice_number',
        label_ru='Номер уведомления о регистрации',
        converter=normalize_contract_number,
        pages=2,
    ))
    _append(fields, find_first(
        document,
        patterns=[r'объект\s+недвижимости,\s*расположенн\w*\s+по\s+адресу:\s*([^,\n]+(?:,\s*[^,\n]+){3,8}?)(?=,\s*с\s+кадастровым)'],
        name='property_address',
        label_ru='Адрес недвижимости',
        converter=lambda value: re.sub(r'\s+', ' ', value).strip(' ,.;'),
        pages=2,
    ))
    _append(fields, find_first(
        document,
        patterns=[r'кадастровым\s+номером\s+([0-9А-Яа-яA-Za-z:.\s-]+?)(?=,\s*сообщает|\s+сообщает)'],
        name='cadastral_number',
        label_ru='Кадастровый номер',
        converter=lambda value: re.sub(r'\s+', '', value).strip(' ,.;'),
        pages=2,
    ))
    _append(fields, find_first(
        document,
        patterns=[r'(объект\s+недвижимости)'],
        name='property_type',
        label_ru='Вид недвижимости',
        converter=lambda _: 'Недвижимое имущество',
        pages=2,
    ))
    return _deduplicate(fields)
...
def parse_by_type(document: ReadDocument, doc_type: str) -> list[dict]:
    parsers = {
        'real_estate_registration_notice': parse_real_estate_registration_notice,
        ...
        'guarantee_contract': parse_guarantee_contract,
        'direct_debit_agreement': parse_direct_debit_agreement,
        'bank_guarantee_application': parse_bank_guarantee_application,
