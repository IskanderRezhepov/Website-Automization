...
AMOUNT_FIELD_NAMES = {
    ...
    "new_principal_amount_kzt": "Новая сумма основного долга",
    "minimum_commission_kzt": "Минимальная комиссия",
    "advance_payment_kzt": "Авансовый платёж",
    "gps_equipment_total_kzt": "Стоимость GPS-оборудования",
    "gps_annual_fee_kzt": "Абонентская плата GPS за год",
    "gps_service_fee_kzt": "Стоимость GPS-услуг",
    "insurance_sum_kzt": "Страховая сумма",
    "insurance_premium_kzt": "Страховая премия",
}
...
def _field_formula_checks(document: dict) -> list[dict]:
    usable = {
        item.get("name"): item
        for item in _usable_fields(document)
    ...
    def amount(name: str) -> Decimal | None:
        return _decimal((usable.get(name) or {}).get("value"))

    insurance_sum = amount("insurance_sum_kzt")
    premium = amount("insurance_premium_kzt")
    tariff = amount("insurance_tariff_percent")
    if insurance_sum is not None and premium is not None and tariff is not None:
        expected = insurance_sum * tariff / Decimal("100")
        checks.append(_compare(
            "Страховая премия: сумма × тариф",
            "Расчётная премия", expected,
            "Указанная премия", premium,
            [
                _table_evidence(document, "Страховая сумма × тариф", expected),
                _table_evidence(document, "Страховая премия", premium),
            ],
        ))

    equipment = amount("gps_equipment_total_kzt")
    annual = amount("gps_annual_fee_kzt")
    gps_total = amount("gps_service_fee_kzt")
    if equipment is not None and annual is not None and gps_total is not None:
        expected = equipment + annual
        checks.append(_compare(
            "GPS: оборудование + абонплата за первый год",
            "Расчётная стоимость GPS", expected,
            "Указанная общая стоимость GPS", gps_total,
            [
                _table_evidence(document, "Оборудование + абонплата", expected),
                _table_evidence(document, "Общая стоимость GPS", gps_total),
            ],
        ))

    asset = next((
        amount(name) for name in (
            "lease_asset_value_kzt", "purchase_total_kzt", "total_amount_kzt",
        ) if amount(name) is not None
    ), None)
    advance = amount("advance_payment_kzt")
    financing = amount("financing_amount_kzt")
    if asset is not None and advance is not None and financing is not None:
        expected = asset - advance
        checks.append(_compare(
            "Финансирование: стоимость − аванс",
            "Расчётная сумма финансирования", expected,
            "Подтверждённая сумма финансирования", financing,
            [
                _table_evidence(document, "Стоимость − аванс", expected),
                _table_evidence(document, "Сумма финансирования", financing),
            ],
        ))
    return checks
...
def build_financial_checks(documents: list[dict]) -> tuple[list[dict], dict]:
    ...
    for document in documents:
        ...
        checks.extend(_field_formula_checks(document))
        for table in document.get("tables", []):
    ...
    observations = []
    for document in documents:
        for name, amount, evidence in _field_amounts(document):
            observations.append({
                "name": name,
                "label_ru": AMOUNT_FIELD_NAMES[name],
                "value_kzt": float(amount),
                "filename": document.get("filename"),
    ...
    summary = {
        ...
        "amount_fields_count": len(observations),
